...
export default function LeadImportModal({ isOpen, onClose, onImported }) {
  ...
  const loadAssignableUsers = async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Unable to load assignable users");
    } finally {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Unable to import leads");
    } finally {
      setIsImporting(false);
    }
  };
  ...
  const handleDownloadFailedRows = async () => {
    ...
    await buildFailedRowsWorkbook(summary.failedRows, fileName || "lead-import");
  };
  ...
  return (
    <Modal title="Import Leads" isOpen={isOpen} onClose={closeModal}>
                ...
                <tbody>
                  {previewRows.map((row) => (
                    <tr key={`${row.rowNumber}-${row.phone}`} className="border-t border-white/10">
                      ...
                      <td className="px-3 py-2 text-muted">{row.leadStatus || "New Lead"}</td>
