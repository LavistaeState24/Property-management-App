  ...
  return (
    <div className="space-y-6">
      ...
      <form className="grid gap-5 rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-glass lg:grid-cols-2" onSubmit={handleSubmit}>
        <FormInput label="Client Name" icon={UserRound} value={form.name} onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))} />
        <FormInput label="Phone" icon={UserRound} value={form.phone} onChange={(event) => setForm((prev) => ({ ...prev, phone: event.target.value }))} />
        <FormInput label="Email" icon={Mail} type="email" value={form.email} onChange={(event) => setForm((prev) => ({ ...prev, email: event.target.value }))} />
        <FormInput label="Requirement" value={form.requirement} onChange={(event) => setForm((prev) => ({ ...prev, requirement: event.target.value }))} />
        <FormInput label="Budget Min" icon={Wallet} value={form.budgetMin} onChange={(event) => setForm((prev) => ({ ...prev, budgetMin: event.target.value }))} />
        <FormInput label="Budget Max" icon={Wallet} value={form.budgetMax} onChange={(event) => setForm((prev) => ({ ...prev, budgetMax: event.target.value }))} />
        <FormInput label="Preferred Area" icon={MapPin} value={form.preferredArea} onChange={(event) => setForm((prev) => ({ ...prev, preferredArea: event.target.value }))} />
        ...
        <FormInput label="Follow-up Date" icon={CalendarDays} type="date" value={form.followUpDate} onChange={(event) => setForm((prev) => ({ ...prev, followUpDate: event.target.value }))} />
        ...
        <div className="lg:col-span-2">
          <Button disabled={submitting} icon={Save}>{submitting ? "Saving..." : "Save Client"}</Button>
        </div>
      </form>
    </div>
  );
