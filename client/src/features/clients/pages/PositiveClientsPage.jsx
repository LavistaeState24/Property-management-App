...
export default function PositiveClientsPage() {
  ...
  const loadPositiveClients = async (nextFilters = initialFilters) => {
    ...
    try {
      ...
      setListError(requestError.response?.data?.message || "Unable to load positive clients");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) =>
        row.interestedProjects?.length ? (
          ...
          <span className="text-muted">No shared projects</span>
        ),
    },
    {
      key: "objections",
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-wrap gap-2">
          <Badge tone={getLeadStatusTone(row.leadStatus)}>{row.leadStatus || "New Lead"}</Badge>
