...
export default function DealUpsertPage() {
  ...
  useEffect(() => {
    const loadDependencies = async () => {
      ...
      try {
        ...
        setLoadError(requestError.response?.data?.message || "Unable to load deal form");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs uppercase tracking-[0.3em] text-gold">{isEditMode ? "Edit Deal" : "New Deal"}</p>
