...
const infoItems = (deal) => [
  ["Lead", deal.lead?.ownerName || "-"],
  ["Lead Phone", deal.lead?.clientPhoneNumber || "-"],
  ["Project", deal.project?.projectName || "-"],
  ...
  useEffect(() => {
    const loadDeal = async () => {
      try {
        ...
        setLoadError(requestError.response?.data?.message || "Unable to load deal details");
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-gold">Deal Details</p>
          <h2 className="mt-2 font-display text-4xl">{deal.lead?.ownerName || "Deal"}</h2>
          <p className="mt-2 text-sm text-muted">{deal.project?.projectName || "Project not attached"}</p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          {canUpdateDeals ? (
            <Link to={`/deals/${id}/edit`}>
              <Button icon={Pencil}>Edit Deal</Button>
            </Link>
          ) : null}
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-glass">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-display text-2xl">Deal Information</h3>
          </div>
