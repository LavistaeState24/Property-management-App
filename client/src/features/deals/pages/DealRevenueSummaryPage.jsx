...
export default function DealRevenueSummaryPage() {
  ...
  const loadReports = async (nextFilters = filters) => {
    ...
    try {
      ...
      setLoadError(requestError.response?.data?.message || "Unable to load revenue summary");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="mt-2 font-display text-3xl">Revenue Summary</h2>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total Deals" value={summary?.totalDeals ?? "--"} accent="gold" meta="Tracked" />
        <StatCard label="Closed Deals" value={summary?.closedDeals ?? "--"} accent="green" meta="Closed" />
