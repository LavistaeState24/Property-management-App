...
export default function DashboardPage() {
  ...
  const projectColumns = [
    {
      ...
      label: "Project",
      searchValue: (row) =>
        `${row.projectName} ${row.publicAlias}`,
      render: (row) => (
        <div>
    ...
    },
    {
      ...
      label: "Config",
      searchValue: (row) =>
        `${row.configuration || ""} ${formatPropertyTypes(
          row.propertyType,
        )}`,
      render: (row) =>
        row.configuration ||
  ...
  return (
    <div className="space-y-6 text-heading">
      ...
      {canViewDealReports ? (
        <section className="space-y-4 rounded-[32px] border border-border bg-surface p-6 ">
          <div>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1">
        {canViewClients ? (
          <div className="space-y-6">
            <div className="rounded-[32px] border border-border bg-surface p-6 ">
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
                {clients.map((client) => (
                  <ClientCard
                    key={client._id}
                    client={client}
