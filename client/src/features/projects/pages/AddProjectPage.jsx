  ...
  return (
    <div className="space-y-6">
      ...
      <form className="grid gap-5 rounded-[32px] border border-white/10 bg-white/5 p-6 shadow-glass lg:grid-cols-2" onSubmit={handleSubmit}>
        ...
        <FormInput label="Client-safe Alias"
          icon={Building2}
          placeholder="Enter client-safe alias"
          value={form.publicAlias}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, publicAlias: event.target.value }))} />
        ...
        <SelectDropdown label="Property Type"
          ...
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, propertyType: event.target.value }))} />
        ...
        <FormInput label="Configuration"
          ...
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, configuration: event.target.value }))} />

        <FormInput label="Size Min"
          placeholder="Minimum size (sq ft)"
          value={form.sizeRange.min}
          onChange={(event) => updateNested("sizeRange", "min", event.target.value)} />

        <FormInput label="Size Max"
          placeholder="Maximum size (sq ft)"
          value={form.sizeRange.max}
          onChange={(event) => updateNested("sizeRange", "max", event.target.value)} />

        <FormInput label="Price Min"
          icon={Wallet}
          placeholder="Minimum price"
          value={form.priceRange.min}
          onChange={(event) => updateNested("priceRange", "min", event.target.value)} />

        <FormInput label="Price Max"
          icon={Wallet}
          placeholder="Maximum price"
          value={form.priceRange.max}
          onChange={(event) => updateNested("priceRange", "max", event.target.value)} />

        <FormInput label="Total Plot Size"
          placeholder="Enter total plot size (sq yd)"
          value={form.totalPlotSize}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, totalPlotSize: event.target.value }))} />

        <FormInput label="Total Blocks"
          placeholder="Enter number of blocks"
          value={form.totalBlocks}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, totalBlocks: event.target.value }))} />

        <FormInput label="Total Units"
          placeholder="Enter total units"
          value={form.totalUnits}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, totalUnits: event.target.value }))} />

        <FormInput label="Available Units"
          placeholder="Enter available units"
          value={form.availableUnits}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, availableUnits: event.target.value }))} />

        <FormInput label="Possession Date"
          icon={CalendarDays}
          type="date"
          value={form.possessionDate}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, possessionDate: event.target.value }))} />
        ...
        <FormInput label="Amenities"
          ...
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, amenities: event.target.value }))} />

        <FormInput label="Sample House Video URL"
          className="lg:col-span-2"
          placeholder="Paste YouTube / Drive link"
          value={form.sampleHouseVideoUrl}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, sampleHouseVideoUrl: event.target.value }))} />

        <FormInput label="Builder Details"
          className="lg:col-span-2"
          placeholder="Enter builder / developer details"
          value={form.builderDetails}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, builderDetails: event.target.value }))} />

        <FormInput label="Internal Notes"
          className="lg:col-span-2"
          placeholder="Add internal notes (not visible to client)"
          value={form.internalNotes}
          onChange={(event) =>
            setForm((prev) =>
              ({ ...prev, internalNotes: event.target.value }))} />
