...
export const validateProjectInput = (payload) => {
  ...
  const hasSampleVideo =
    payload.hasSampleVideo === true ||
    payload.hasSampleVideo === "true" ||
    payload.hasSampleVideo === 1 ||
    payload.hasSampleVideo === "1";

  const sizeMin = validateNumber(errors, "sizeRange.min", payload.sizeRange?.min, {
    label: "Minimum size",
    required: true,
    min: 1,
  });
  const sizeMax = validateNumber(errors, "sizeRange.max", payload.sizeRange?.max, {
    label: "Maximum size",
    required: true,
    min: 1,
  });
  ...
  const priceMax = validateNumber(errors, "priceRange.max", payload.priceRange?.max, {
    label: "Maximum price",
    required: true,
    min: 1,
  });
  const totalUnits = validateNumber(errors, "totalUnits", payload.totalUnits, {
    label: "Total units",
    required: true,
    min: 1,
    integer: true,
  ...
  if (hasSampleVideo) {
    sampleVideoUrl = validateOptionalUrl(errors, "sampleVideoUrl", payload.sampleVideoUrl, "Video URL");

    if (!sampleVideoUrl) {
      errors.sampleVideoUrl = "Video URL is required";
    }
  } else {
    sampleVideoUrl = undefined;

    if (payload.sampleVideoUrl) {
      errors.sampleVideoUrl = "Video URL must be empty when no video is available";
    }
  }
  ...
  if (payload.brochure) {
    const brochureName = validateOptionalText(errors, "brochure.name", payload.brochure.name, {
      label: "Brochure name",
      min: 1,
      max: 120,
    });
    const brochureUrl = validateOptionalAssetUrl(errors, "brochure.url", payload.brochure.url, "Brochure URL");

    if (!brochureName) {
      errors["brochure.name"] = "Brochure name is required";
    }

    if (!brochureUrl) {
      errors["brochure.url"] = "Brochure URL is required";
    }

    brochure = brochureName && brochureUrl ? { name: brochureName, url: brochureUrl } : undefined;
  }
